package com.coding.challenges.codingame.unleashthegeek;
...
enum EAction {
  ...
  KAMIKAZE_BLOW_UP,
  SINGLE_KAMIKAZE,
  SINGLE_KAMIKAZE_PLANT,
  SINGLE_KAMIKAZE_BLOW_UP
}
...
class Board {
  ...
  void update(Scanner in) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (!cells[y][x].dugByEnnemy
            && cells[y][x].known
            && cells[y][x].ore > 0
            && !myTrapPos.contains(new Coord(x, y))) {
        ...
        if (cells[y][x].hole != prevCells[y][x].hole) {
          ...
          for (Entity robot : myTeam.robots) {
            if ((robot.action.command.equals("MOVE") | robot.action.command.equals("DIG"))
                && robot.pos.equals(newHolePos)) {
  ...
  public boolean shouldGetRadar() {
    System.err.println(
        (totOreAvailable <= maxOresBeforeGettingRarar)
            + " - "
            + !gettingRadar
            + " - "
            + (myRadarCooldown == 0));
...
class Player {
    ...
    while (true) {
      ...
      for (Entity robot : board.myTeam.robots) {
        if (robot.isAlive()) {
          ...
          else if (board.state == GameState.USE_LINE_KAMIKAZE
              && board.getId(robot.id) == board.kamikazeId
          ...
          else if (board.state == GameState.USE_SINGLE_KAMIKAZE
              && board.getId(robot.id) == board.kamikazeId
  ...
  public boolean makeThingsGoBoom(Board board) {
    ...
    for (Entity enemyRobot : board.opponentTeam.robots) {
      if ((enemyRobot.pos.x <= 2
              && enemyRobot.pos.y <= lowestTrapPos.y
              && enemyRobot.pos.y >= board.startTrapPosY)
    ...
    for (Entity myRobot : board.myTeam.robots) {
      if ((myRobot.pos.x <= 2
              && myRobot.pos.y <= lowestTrapPos.y
              && myRobot.pos.y >= board.startTrapPosY)
    ...
    System.err.println(
        "My:"
            ...
            + enemyCasualties
            + " - final:"
            + ((enemyCasualties - myCasualties) > 0));
    return ((enemyCasualties - myCasualties) > 0);
  }
  ...
  public void handleSingleKamikaze(Entity robot, Board board) {
    System.err.println(
        "id:"
            ...
            + robot.target.id);
    if (robot.target != null
        && !robot.target.isAlive()) { // Target died by itself, getting new target
      for (Entity opponentRobot : board.opponentTeam.robots) {
        if (opponentRobot.isAlive()) {
          robot.target = opponentRobot;
          break;
    ...
    if (robot.target != null
        ...
        && makeThingGoBoom(board, robot, robot.target.pos)) {
      if (explodeUnit(robot, board)) {
    ...
    switch (robot.item) {
      case TRAP: // Plant trap if opponent coming back
        if ((robot.target.prevPos.x - robot.target.pos.x) > 2
            && robot.target.prevPos.y == robot.target.pos.y
            && robot.target.pos.x > 4) { // Robot coming back
          robot.prevAction = EAction.SINGLE_KAMIKAZE_PLANT;
          robot.action = Action.dig(new Coord(1, robot.target.pos.y));
          return;
        }
      ...
      case NOTHING: // Go mine closest vine
        if (board.totOreAvailable > 0) {
          ...
          if (robot.action.pos.x == 0) {
            robot.action =
                Action.dig(
                    BFSFindOreIfNoneAvaiable(robot, board, new Coord(board.midX, robot.pos.y)));
          }
        } else {
          robot.action =
              Action.dig(
  ...
  public Coord BFSFindOre(Entity robot, Board board, int minOre) {
    ...
    while (!coordQueue.isEmpty()) {
      ...
      if (board.getCell(currentPos).ore >= minOre
          && !board.myTrapPos.contains(currentPos)
          && !board.getCell(currentPos).dugByEnnemy) {

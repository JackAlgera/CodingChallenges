package com.coding.challenges.codingame.oceanofcode;
        ...
        case 1:
          ...
          for (int i = 0; i < neighborsRow.length; i++) {
            ...
            if (theBoard.isOnBoard(neighborRow, neighborColumn)) {
              tempBoard[neighborRow][neighborColumn] =
                  mightHaveOppBoard[neighborRow][neighborColumn];
            }
          }
...
class MasterMind {
  ...
  public void moveShip() {
    ...
    for (int i = 0; i < EDirection.values().length; i++) {
      EDirection desiredDirection =
          EDirection.getEnum((5 - i + lastDirection.getDirectionInt()) % 4);
      ...
      if (gameBoard.isOnBoard(desiredRow, desiredColumn)
          && !gameBoard.tiles[desiredRow][desiredColumn].visited) {
  ...
  public void useSonar() {
    ...
    for (int i = 0; i < gameBoard.oppFinder.isInSection.length; i++) {
      if (gameBoard.oppFinder.isInSection[i]
          && gameBoard.oppFinder.nbrTilesPerSection[i] > nbrTilesTargetSection) {
        targetSection = i + 1;

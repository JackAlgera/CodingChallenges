...
class Player {
  ...
  static class Brain {
    ...
    static Pair<Float> LINE_POS_ONE_TWO_DIVIDER =
        new Pair<>(2.0f * BOTTOM_RIGHT.y / ((float) BOTTOM_RIGHT.x), 0f);
    static Pair<Float> LINE_PLAYER_OPP_DIVIDER =
        new Pair<>(-3.0f * BOTTOM_RIGHT.y / ((float) BOTTOM_RIGHT.x), 2.0f * BOTTOM_RIGHT.y);
    ...
    static Pair<Integer> PATROL_POSITION_OPP_SIDE =
        new Pair<>((int) (BOTTOM_RIGHT.x * 0.6f), (int) (BOTTOM_RIGHT.y * 0.5f));
    ...
    static Pair<Integer> WAITING_POS_ONE =
        new Pair<>((int) (RADIUS * 0.6f), (int) (RADIUS * 1.35f));
    static Pair<Integer> WAITING_POS_TWO =
        new Pair<>((int) (RADIUS * 1.35f), (int) (RADIUS * 0.6f));
    ...
    List<Monster> monstersOnOpponentSide,
        monstersHeadingTowardsOpponentBase,
        monstersInsideOpponentBase;
    ...
    public Brain(Scanner in) {
      ...
      this.opponent =
          new Base(
              IS_PLAYER_BOT_RIGHT ? 0 : BOTTOM_RIGHT.x, IS_PLAYER_BOT_RIGHT ? 0 : BOTTOM_RIGHT.y);

      if (IS_PLAYER_BOT_RIGHT) {
        WAITING_POS_ONE = getOppositePos(WAITING_POS_ONE);
        WAITING_POS_TWO = getOppositePos(WAITING_POS_TWO);
        PATROL_POSITION_OPP_SIDE =
            new Pair<>(BOTTOM_RIGHT.x - PATROL_POSITION_OPP_SIDE.x, PATROL_POSITION_OPP_SIDE.y);
    ...
    public void updateLoop(Scanner in) {
      ...
      for (int i = 0; i < entityCount; i++) {
        ...
        int y = in.nextInt();
        int shieldLife =
            in.nextInt(); // Ignore for this league; Count down until shield spell fades
        int isControlled =
            in.nextInt(); // Ignore for this league; Equals 1 when this entity is under a control
        // spell
        ...
        int threatFor =
            in.nextInt(); // Given this monster's trajectory, is it a threat to 1=your base,
    ...
    public void setupParams() {
      ...
      for (Monster monster : monsters) {
        ...
        if ((IS_PLAYER_BOT_RIGHT && monster.pos.y > yValue)
            || (!IS_PLAYER_BOT_RIGHT && monster.pos.y < yValue)) {
      ...
      oppHeroesInsideBase =
          opponent.heroes.stream()
              .filter(oppHero -> getDistance(oppHero.pos, opponent.pos) < RADIUS)
    ...
    public void determineDefendingHeroStates() {
      if (!monstersInsideBase.isEmpty()) {
        Monster closestMonsterToBase =
            getClosestMonster(player.pos, monstersInsideBase, true, true, false, true);
        ...
        if (closestMonsterToBase.shieldLife == 0) {
          ...
          if (currentTurn > TURN_START_SENDING_MONSTERS_FROM_PLAYER_BASE
              && player.mana > 80
              && closestMonsterToBase.health > 9
              && distanceMonsterToHero < WIND_SPELL_HERO_DISTANCE) {
            player.castSpell(
                closestHero,
                new WindSpellAction(
                    closestHero, DIRECTION_AWAY_FROM_BASE, "WIND " + closestMonsterToBase.id),
                actionsToPerform);
          } else if (player.canCastUrgentSpell()
              && distanceClosestMonsterToBase - MONSTER_SPEED
                  <= MONSTER_PUSH_BACK_FROM_BASE_RANGE) {
            if (distanceMonsterToHero < WIND_SPELL_HERO_DISTANCE) {
              player.castSpell(
                  ...
                  new WindSpellAction(
                      closestHero, DIRECTION_AWAY_FROM_BASE, "WIND " + closestMonsterToBase.id),
                  actionsToPerform);
            } else if (distanceMonsterToHero < MAX_SPELL_RANGE
                && closestMonsterToBase.isControlled != 1) {
              player.castSpell(
                  closestHero,
                  new ControlSpellAction(closestMonsterToBase, getRandomSendMonstersPos()),
                  actionsToPerform);
            }
          }

          actionsToPerform.putIfAbsent(
              closestHero.id,
              new MoveAction(
                  getPositionToAttack(closestMonsterToBase, monsters), closestMonsterToBase.id));
          closestMonsterToBase.isAttacked = true;
        } else if ((distanceClosestMonsterToBase / MONSTER_SPEED) * 2
            < closestMonsterToBase.health) {
          playerDefendingHeroes.forEach(
              hero -> {
                actionsToPerform.putIfAbsent(
                    hero.id,
                    new MoveAction(
                        getPositionToAttack(closestMonsterToBase, monsters),
                        closestMonsterToBase.id));
    ...
    public void determineSingleHeroAction(Hero hero) {
      switch (hero.state) {
        ...
        case PATROL_PLAYER:
          if (hero.isPosOne) {
            actionsToPerform.putIfAbsent(
                hero.id, new MoveAction(getRandomPosAround(WAITING_POS_ONE)));
          } else {
            actionsToPerform.putIfAbsent(
                hero.id, new MoveAction(getRandomPosAround(WAITING_POS_TWO)));
          }
          break;
        ...
        case ATTACK_RANDOM_OPPONENT_MONSTERS:
          Monster closestMonster =
              getClosestMonster(
                  hero.pos,
                  new ArrayList<>(allMonstersOppSide.values()),
                  true,
                  true,
                  hero.isPosOne,
                  true);
          actionsToPerform.putIfAbsent(
              hero.id,
              new MoveAction(getPositionToAttack(closestMonster, monsters), closestMonster.id));
          break;
        case PATROL_CENTER_OPPONENT:
          actionsToPerform.putIfAbsent(
              hero.id, new MoveAction(getRandomPosAround(PATROL_POSITION_OPP_SIDE)));
          break;
    ...
    private boolean handleMonstersIfNeeded(
        Hero hero,
        List<Monster> monsters,
        Pair<Integer> closestPosTo,
        boolean ignoreIsPosOneCheck) {
      if (!monsters.isEmpty()) {
        if (player.canCastSpell()
            && currentTurn > TURN_START_SENDING_MONSTERS_FROM_PLAYER_BASE
            && player.mana > 80) {
          Monster furthestMonster = getFurthestMonsterInRangeEnoughHealth(hero.pos, monsters);
          if (furthestMonster != null) {
            player.castSpell(
                hero,
                new ControlSpellAction(furthestMonster, getRandomSendMonstersPos()),
                actionsToPerform);
            return true;
          }
        }
        ...
        Monster closestMonster =
            getClosestMonster(
                closestPosTo, monsters, false, ignoreIsPosOneCheck, hero.isPosOne, true);
    ...
    public Monster getFurthestMonsterInRangeEnoughHealth(
        Pair<Integer> pos, List<Monster> monsters) {
    ...
    public Monster getClosestMonster(
        Pair<Integer> pos,
        List<Monster> monsters,
        boolean ignoreTargetedMonsters,
        boolean ignoreIsPosOneCheck,
        boolean isPosOne,
        boolean ignoreInRangeCheck) {
  ...
  enum EAttackState {
    ...
    GET_SHIELD,
    SCOUTING,
    CONTROL_MONSTER_TO_POSITION,
    PREPARE_PUSH_POSITION,
    CONTROL_OPPONENT_AWAY,
    PUSH_MONSTER
  }
      ...
      switch (currentState) {
        case NOT_READY:
          ...
          if (distanceHeroCenter < Brain.RADIUS * 0.7) {
            Monster closestMonster =
                brain.getClosestMonster(
                    hero.pos, brain.monstersInsideOpponentBase, true, true, true, false);
            if (closestMonster != null) {
              reset();
              n = 2;
        ...
        case GET_SHIELD:
          ...
          break;
        case SCOUTING:
          boolean shouldGetShield =
              brain.oppHeroesInsideBase.stream()
                      .filter(
                          oppHero ->
                              brain.getDistance(hero.pos, oppHero.pos)
                                  < Brain.WIND_SPELL_HERO_DISTANCE)
                      .count()
                  != 0;
          ...
          Monster closestMonster =
              brain.getClosestMonster(
                  hero.pos, brain.monstersHeadingTowardsOpponentBase, true, true, true, false);
          ...
          closestMonster =
              brain.getClosestMonster(
                  hero.pos, brain.monstersOnOpponentSide, true, true, true, false);
          if (closestMonster != null) {
            reset();
        ...
        case CONTROL_MONSTER_TO_POSITION:
          if (targetStillGood(brain, hero)) {
            currentState = EAttackState.PREPARE_PUSH_POSITION;
            brain.player.castSpell(
                hero,
                new ControlSpellAction(targetMonster, Brain.CONTROL_MONSTER_TO_POSITION_ONE),
                brain.actionsToPerform);
          }
          break;
        ...
        case PUSH_MONSTER:
          if (targetStillGood(brain, hero)) {
            ...
            brain.player.castSpell(
                ...
                new WindSpellAction(
                    hero,
                    new Pair<>(directVect.x * 1200f, directVect.y * 1200f),
                    "Wind " + nbrOfPushes),
                brain.actionsToPerform);
            break;
          }
        ...
        case CONTROL_OPPONENT_AWAY:
          ...
          brain.player.castSpell(
              hero, new ControlSpellAction(bestOppHero, brain.player.pos), brain.actionsToPerform);
          break;
      }
    }

    public Pair<Integer> getPPositionInNTurns(Brain brain, Monster monster, int n) {
      Pair<Integer> monsterPosN =
          new Pair<>(monster.pos.x + n * monster.vx, monster.pos.y + n * monster.vy);
      ...
      return new Pair<>(
          (int) (monsterPosN.x + directVect.x * 1200f),
          (int) (monsterPosN.y + directVect.y * 1200f));
    }
  ...
  enum EHeroState {
    ATTACK_RANDOM_OPPONENT_MONSTERS,
    STATE_ALREADY_SET,
    DEFEND_BASE,
    PATROL_PLAYER,
    PATROL_CENTER_OPPONENT,
    DESTROY_OPPONENT
  }
  ...
  static class WindSpellAction extends BaseAction {
    ...
    void performAction() {
      System.out.printf(
          "SPELL WIND %d %d %s%n",
          (int) (pos.x + direction.x), (int) (pos.y + direction.y), message);
    }
  }

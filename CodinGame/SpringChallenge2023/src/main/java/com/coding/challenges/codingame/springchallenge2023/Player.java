package com.coding.challenges.codingame.springchallenge2023;
...
enum GameState {
  GET_EGGS_FIRST_LAYER,
  GET_EGGS_SECOND_LAYER,
  GET_CRYSTALS_ENEMY_SIDE,
  GET_CRYSTALS_OWN_SIDE
}
...
class Brain {
  ...
  public void playTurn3() {
    ...
    while (lookingForPath) {
      ...
      int nextCellIndex =
          globals.eggsAndCrystals.values().stream()
              .filter(cell -> !visitedCells.contains(cell.index))
              .min(
                  Comparator.comparingInt(
                      cell -> globals.distanceToCellMap.get(finalCurrentCellIndex).get(cell.index)))
  ...
  //        Queue<NewPathState> queue = new PriorityQueue<>(Comparator.comparingInt(o ->
  // o.heuristic));
  ...
  //
  //            System.err.println("Current: " + current.index + " " + current.heuristic + " " +
  // Arrays.toString(current.path));
  //            if (current.heuristic > bestHeuristic) {
  //                System.err.println("Best so far: " + current.index + " " + current.heuristic + "
  // " + Arrays.toString(current.path));
  ...
  //                    }
  //
  //                    queue.add(new NewPathState(neighborIndex, getHeuristic(neighborIndex,
  // minResource, current.path), newPath, minResource));
  ...
  //                  .filter(egg ->
  // globals.distanceToCellMap.get(globals.myBases[0]).get(egg.index) == 1)
  ...
  //                } else {
  //                    swapState(GameState.GET_EGGS_SECOND_LAYER, "No eggs in first layer, swapping
  // to second layer");
  //                    return;
  //                }
  //                break;
  ...
  //                  .filter(egg -> egg.resources > 0)
  //                  .filter(egg ->
  // globals.distanceToCellMap.get(globals.myBases[0]).get(egg.index) == 2)
  //                  .map(egg -> Arrays.asList(new Line(globals.myBases[0], egg.index, 1), new
  // Beacon(egg.index, 2)))
  ...
  //                    actions.addAll(newActions);
  //                } else {
  ////                    swapState(GameState.GET_CRYSTALS_ENEMY_SIDE, "No eggs in second layer,
  // getting crystals on enemy side");
  //                    swapState(GameState.GET_CRYSTALS_OWN_SIDE, "No eggs in second layer, getting
  // closest crystals");
  //                    return;
  //                }
  //                break;
  ...
  // globals.distanceToCellMap.get(globals.myBases[0]).get(cell.index)))
  //                  .limit(2)
  ////                  .map(cell -> Arrays.asList(new Line(globals.myBases[0], cell.index, 1), new
  // Beacon(cell.index, 2)))
  ////                  .flatMap(Collection::stream)
  //                  .map(egg -> new Line(globals.myBases[0], egg.index, 1))
  //                  .collect(Collectors.toList()));
...
class Cell {
  ...
  public String toString() {
    return String.format(
        "Cell{index=%d, type=%s, resources=%d, myAnts=%d, oppAnts=%d}",
        index, type, resources, myAnts, oppAnts);
  }
}
...
class Globals {
  ...
  public Globals(Scanner in) {
    ...
    this.myBases = IntStream.range(0, numberOfBases).map(i -> in.nextInt()).toArray();
    this.oppBases = IntStream.range(0, numberOfBases).map(i -> in.nextInt()).toArray();
  ...
  private void setCellDistances(int startIndex) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int neighbor : neighborMap.get(current.index).values()) {
        ...
        if (distanceToCellMap.get(startIndex).get(neighbor) == -1
            || newDistance < distanceToCellMap.get(startIndex).get(neighbor)) {

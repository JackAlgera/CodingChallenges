package com.coding.challenges.adventofcode.year2023.Day05;
  ...
  public Set<SeedRange> playStep(Set<SeedRange> seedRanges, List<MapRange> mapRanges) {
    ...
    for (MapRange mapRange : mapRanges) {
      ...
      for (SeedRange seedRange : seedRanges) {
        ...
        checkedSeeds.add(
            new SeedRange(
                mapRange.destinationStart()
                    + (Math.max(mapRange.sourceStart(), seedRange.start())
                        - mapRange.sourceStart()),
                mapRange.destinationEnd()
                    + (Math.min(mapRange.sourceEnd(), seedRange.end()) - mapRange.sourceEnd())));
    ...
    return Stream.concat(checkedSeeds.stream(), temp.stream()).collect(Collectors.toSet());
  }

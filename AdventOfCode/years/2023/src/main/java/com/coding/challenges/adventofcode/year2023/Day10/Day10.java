...
public class Day10 extends Day<Integer> {
  ...
  public final List<List<String>> OUTWARD_PIPES =
      List.of(NORTH_PIPES, EAST_PIPES, SOUTH_PIPES, WEST_PIPES);
  public final List<List<String>> INWARD_PIPES =
      List.of(SOUTH_PIPES, WEST_PIPES, NORTH_PIPES, EAST_PIPES);
  ...
  public char convertS(int i, int j, Node[][] grid) {
    ...
    if (i == 0) {
      ...
      if (SOUTH_PIPES.contains(grid[i - 1][j].type) && WEST_PIPES.contains(grid[i][j + 1].type)) {
        c = 'L';
      } else if (WEST_PIPES.contains(grid[i][j + 1].type)
          && NORTH_PIPES.contains(grid[i + 1][j].type)) {
        c = 'F';
      } else if (NORTH_PIPES.contains(grid[i + 1][j].type)
          && EAST_PIPES.contains(grid[i][j - 1].type)) {
        c = '7';
      } else if (EAST_PIPES.contains(grid[i][j - 1].type)
          && SOUTH_PIPES.contains(grid[i - 1][j].type)) {
        c = 'J';
      } else {
        c = '|';
      }

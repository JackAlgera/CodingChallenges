...
public class Day7 extends Day<Long> {
    ...
    public Long getTotalWinnings(List<String> lines, boolean useJokers) {
        ...
        lines.stream()
          .map(line -> line.split(" "))
          .map(hand -> new Hand(hand[0], Long.parseLong(hand[1]), getHandStrength(hand[0], useJokers)))
          .forEach(hand -> handTypes.get(hand.cardType().ordinal()).add(hand));

        List<Hand> handsList = handTypes.stream()
          .flatMap(hands -> hands.stream()
            .sorted((h1, h2) -> h1.compare(h2, useJokers)))
          .toList();

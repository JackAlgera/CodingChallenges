package com.coding.challenges.adventofcode.year2023.Day25;
...
public class Day25 extends Day<Long> {
  ...
  public Long part1(List<String> lines) {
    ...
    while (true) {
      ...
      for (String nodeId : leftGroup) {
        int count =
            (int) nodes.get(nodeId).children().stream().filter(rightGroup::contains).count();
        if (count > max) {
          max = count;
          maxId = nodeId;
        }
  ...
  public Map<String, Node> parseInput(List<String> lines) {
    ...
    for (String line : lines) {
      ...
      Set<String> children =
          Arrays.stream(line.split(": ")[1].split(" ")).collect(Collectors.toSet());

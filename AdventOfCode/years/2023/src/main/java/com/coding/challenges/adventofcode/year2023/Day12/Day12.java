package com.coding.challenges.adventofcode.year2023.Day12;
...
public class Day12 extends Day<Long> {
  ...
  public long countArrangements(
      State state, String springs, List<Integer> groups, Map<State, Long> dp) {
    ...
    if (state.springIndex == springs.length()) {
      ...
      if (state.groupsIndex == groups.size() - 1
          && state.groupSize == groups.get(state.groupsIndex)) {
        return 1;
      }
      return 0;
    }
    ...
    if (c == '#') {
      tot +=
          countArrangements(
              state.next(state.groupsIndex, state.groupSize + 1), springs, groups, dp);
    }

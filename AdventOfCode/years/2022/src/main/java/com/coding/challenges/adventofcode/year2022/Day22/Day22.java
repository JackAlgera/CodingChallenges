package com.coding.challenges.adventofcode.year2022.Day22;
...
public class Day22 extends Day<Integer> {
  ...
  private final List<Area> SAMPLE_AREAS =
      List.of(
          new Area(0, 0, 4, 8, 12),
          new Area(1, 4, 8, 0, 4),
          new Area(2, 4, 8, 4, 8),
  ...
  // min included, max excluded
  private final List<Area> INPUT_AREAS =
      List.of(
  ...
  public Me moveWrapAroundInput(Me me, TileType[][] grid) {
    ...
    int areaId =
        INPUT_AREAS.stream().filter(a -> a.isInArea(me.i(), me.j())).findFirst().get().id();
    ...
    if (grid[newI][newJ] == TileType.WALL) {
      ...
      return new Me(newI, newJ, me.direction());
    }
  }
  ...
  public Me moveWrapAroundSample(Me me, TileType[][] grid) {
    ...
    int areaId =
        SAMPLE_AREAS.stream().filter(a -> a.isInArea(me.i(), me.j())).findFirst().get().id();
    Me potentialMe = me.move();
  ...
  public Me moveCubeInput(Me me, TileType[][] grid) {
    ...
    if (grid[newI][newJ] == TileType.WALL) {
      ...
      return new Me(newI, newJ, newDirection);
    }
  }
  ...
  public List<String> extractActions(List<String> lines) {
    ...
    Matcher matcher = Pattern.compile("(\\d+|[A-Z])").matcher(lines.get(lines.size() - 1));
    while (matcher.find()) {
  ...
  public record Me(int i, int j, Direction direction) {
    public Me rotate(char r) {
      return new Me(
          ...
          j,
          Direction.values()[
              Math.floorMod(direction.ordinal() + (r == 'R' ? 1 : -1), Direction.values().length)]);
    }
    ...
    public Me move() {
      ...
      return new Me(newI, newJ, direction);
    }

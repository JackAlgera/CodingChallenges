package com.coding.challenges.adventofcode.year2022.Day11;
...
public class Day11 extends Day<Long> {
    ...
    for (int k = 0; k < totalMonkeys; k++) {
      ...
      monkeys.add(
          new Monkey(
              ...
              new ArrayDeque<>(
                  Arrays.stream(monkeyLines.get(1).split(":")[1].split(","))
                      .map(val -> Long.parseLong(val.strip()))
                      .toList()),
              monkeyLines.get(2).contains("*")
                  ? OperationType.MULTIPLICATION
                  : OperationType.ADDITION,
              monkeyLines.get(2).split(" ")[7].contains("old")
                  ? -1
                  : Integer.parseInt(monkeyLines.get(2).split(" ")[7].strip()),
  ...
  public record Monkey(
      ...
      int operationValue,
      int divisibleBy,
      int ifTrueMonkeyIndex,
      int ifFalseMonkeyIndex) {

package com.coding.challenges.adventofcode.year2022.Day16;
...
public class Day16 extends Day<Integer> {
  ...
  public static void main(String[] args) throws IOException {
    ...
    day.printPart1("sample-input", 1651);
    day.printPart1("input", 2114);

    day.printPart2("sample-input", 1707);
    day.printPart2("input", 2666);
  }
  ...
  public Integer part1(List<String> lines) {
    ...
    Map<String, Node> nodeMap =
        lines.stream().map(this::extractValve).collect(Collectors.toMap(Node::id, n -> n));
    Map<String, Map<String, Integer>> shortestDistances = getShortestDistances(nodeMap);
    Map<String, Node> nodesWithFlowMap =
    ...
    return bfs(
        new State(
            ...
            0,
            "AA",
            nodesWithFlowMap.values().stream()
                .map(n -> n.id)
  ...
  public Integer part2(List<String> lines) {
    ...
    for (int i = 0; i < 250; i++) {
      ...
      for (int k = 1; k < nodesWithFlowMap.size() - 1; k++) {
        int pressureElephant =
            bfs(
                new State(0, maxTime, 0, "AA", nodesToCheck.stream().limit(k).toList()),
                maxTime,
                nodeMap,
                shortestDistances);
        ...
        int pressureJack =
            bfs(
                new State(0, maxTime, 0, "AA", nodesToCheck.stream().skip(k).toList()),
                maxTime,
                nodeMap,
                shortestDistances);
  ...
  public int bfs(
      ...
      int maxTime,
      Map<String, Node> nodeMap,
      Map<String, Map<String, Integer>> shortestDistances) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (String node : state.nodesToVisit) {
        ...
        int totalPossiblePressure =
            (state.timeLeft() - timeToStartProducing) * nodeMap.get(node).flowRate;
        queue.add(
            new State(
  ...
  public record State(
      int time, int maxTime, int totalPressure, String node, List<String> nodesToVisit) {
    public int timeLeft() {
      return maxTime - time;
    }
  }
  ...
  private Node extractValve(String input) {
    Matcher matcher =
        Pattern.compile(
                "Valve ([A-Z]+) has flow rate=(\\d+); tunnel(s\\b|\\b) lead(s\\b|\\b) to valve(s\\b|\\b) (.*)")
            .matcher(input);
    if (matcher.find()) {
      return new Node(
          matcher.group(1),

package com.coding.challenges.adventofcode.year2022.Day15;
...
public class Day15 {
  ...
  private void part1() throws IOException {
    ...
    System.out.println("\n-------- Day 15 - Part 1 --------");
    System.out.println(
        "Total positions that cannot contain beacon: " + positionsCannotContainBeacon);
  ...
  private Pair<Integer> getCoveredArea(Pair<Integer> sensor, Pair<Integer> beacon, int line) {
    ...
    return new Pair<>(sensor.getFirst() - delta, sensor.getFirst() + delta);
  }

  private Pair<Integer> getCoveredAreaLimited(
      Pair<Integer> sensor, Pair<Integer> beacon, int line, int max) {
    int delta = distanceBetween(sensor, beacon) - Math.abs(sensor.getSecond() - line);

    return new Pair<>(
        Math.max(sensor.getFirst() - delta, 0), Math.min(sensor.getFirst() + delta, max));
  }

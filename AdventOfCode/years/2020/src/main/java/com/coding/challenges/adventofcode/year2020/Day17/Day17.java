package com.coding.challenges.adventofcode.year2020.Day17;
...
import com.coding.challenges.adventofcode.utils.Vector4d;
...
public class Day17 extends Day<Long> {
      ...
      for (int x = -1; x <= 1; x++) {
        for (int y = -1; y <= 1; y++) {
          for (int z = -1; z <= 1; z++) {
            ...
            neighbors.add(new Vector4d(cube.x() + x, cube.y() + y, cube.z() + z, cube.w() + w));
          }
        }
      }
    }
  ...
  public Set<Vector4d> parseInput4d(List<String> lines) {
    return parseInput3d(lines).stream()
        .map(cube -> new Vector4d(cube.x(), cube.y(), cube.z(), 0))
        .collect(Collectors.toSet());
  }
}

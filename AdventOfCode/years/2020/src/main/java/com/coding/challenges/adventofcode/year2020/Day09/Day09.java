package com.coding.challenges.adventofcode.year2020.Day09;
  ...
  public Long part2(List<String> lines) {
    ...
    for (int i = 0; i < lines.size() - 2; i++) {
      for (int j = i + 2; j < lines.size(); j++) {
        long sum = lines.subList(i, j).stream().mapToLong(Long::parseLong).sum();
        ...
        if (sum == invalidNumber) {
          List<Long> parsedVals =
              lines.subList(i, j).stream().mapToLong(Long::parseLong).sorted().boxed().toList();

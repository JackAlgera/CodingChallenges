package com.coding.challenges.adventofcode.year2020.Day06;
...
public class Day06 extends Day<Long> {
  ...
  private long countAnswers(List<String> lines, boolean countOnlyUnanimous) {
    ...
    while (i < lines.size()) {
      ...
      yeses +=
          countOnlyUnanimous
              ? countUnanimousAnswers(lines.subList(i, i + k))
              : countAllAnswers(lines.subList(i, i + k));
      i += k + 1;
    }
  ...
  public int countAllAnswers(List<String> inputs) {
    return String.join("", inputs).chars().boxed().collect(Collectors.toSet()).size();
  }

...
public class Day14 extends Day<Long> {
  ...
  public void applyFloatingAddress(
      char[] floatingAddress, int index, long currentAddress, long value, Map<Long, Long> memory) {
    ...
    if (floatingAddress[index] == '1' || floatingAddress[index] == 'X') {
      applyFloatingAddress(
          floatingAddress, index + 1, currentAddress + (long) Math.pow(2, index), value, memory);
    }
    if (floatingAddress[index] == '0' || floatingAddress[index] == 'X') {
      applyFloatingAddress(floatingAddress, index + 1, currentAddress, value, memory);
    }
  }

package adventofcode.Year2023.Day20;
...
public class Day20 extends Day<Long> {
    ...
    public Pulses bfs(Map<String, Conjunction> conjunctions, Map<String, FlipFlop> flipFlops, String target) {
    ...
    public void parseInput(List<String> lines, Map<String, Conjunction> conjunctions, Map<String, FlipFlop> flipFlops) {
        ...
        for (FlipFlop flipFlop : flipFlops.values()) {
            flipFlop.destinations().forEach(destination -> {
                if (conjunctions.containsKey(destination)) {
        ...
        for (Conjunction conjunction : conjunctions.values()) {
            conjunction.destinations().forEach(destination -> {
                if (conjunctions.containsKey(destination)) {
    ...
    public record Conjunction(String id, Map<String, PulseType> parentPulses, List<String> destinations) {
        public PulseType pulse() {
            return parentPulses.values().stream().allMatch(pulse -> pulse == PulseType.HIGH) ? PulseType.LOW : PulseType.HIGH;
        }
    }
    @With

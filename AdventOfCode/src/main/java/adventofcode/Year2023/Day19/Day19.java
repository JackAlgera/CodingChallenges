package adventofcode.Year2023.Day19;
...
public class Day19 extends Day<Long> {
    ...
    public Long part1(List<String> lines) throws IOException {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (addingParts) {
                int[] parts = Arrays.stream(lines.get(i).substring(1, lines.get(i).length() - 1).split(","))
                    .map(s -> s.split("=")[1])
                    .mapToInt(Integer::parseInt)
                    .toArray();

                partRangesList.add(new PartRanges(
    ...
    public Long part2(List<String> lines) throws IOException {
        Map<String, RuleSet> ruleSets = parseRuleSets(lines);
        return applyRules(new PartRanges(
                new Range(1, 4000),
                new Range(1, 4000),
                new Range(1, 4000),
                new Range(1, 4000)),
    ...
    public long applyRules(PartRanges partRanges, RuleSet current, Map<String, RuleSet> ruleSets, boolean part1) {
        ...
        for (Rule rule : current.rules()) {
            ...
            if (rule.checkLessThan) {
                switch (rule.part) {
                    case "x" -> {
                        newPartRanges = newPartRanges.withX(new Range(x.min(), Math.min(x.max(), rule.threshold - 1)));
                        x = new Range(Math.max(x.min(), rule.threshold), x.max());
                    }
                    case "m" -> {
                        newPartRanges = newPartRanges.withM(new Range(m.min(), Math.min(m.max(), rule.threshold - 1)));
                        m = new Range(Math.max(m.min(), rule.threshold), m.max());
                    }
                    case "a" -> {
                        newPartRanges = newPartRanges.withA(new Range(a.min(), Math.min(a.max(), rule.threshold - 1)));
                        a = new Range(Math.max(a.min(), rule.threshold), a.max());
                    }
                    case "s" -> {
                        newPartRanges = newPartRanges.withS(new Range(s.min(), Math.min(s.max(), rule.threshold - 1)));
                ...
                switch (rule.part) {
                    case "x" -> {
                        newPartRanges = newPartRanges.withX(new Range(Math.max(x.min(), rule.threshold + 1), x.max()));
                        x = new Range(x.min(), Math.min(x.max(), rule.threshold));
                    }
                    case "m" -> {
                        newPartRanges = newPartRanges.withM(new Range(Math.max(m.min(), rule.threshold + 1), m.max()));
                        m = new Range(m.min(), Math.min(m.max(), rule.threshold));
                    }
                    case "a" -> {
                        newPartRanges = newPartRanges.withA(new Range(Math.max(a.min(), rule.threshold + 1), a.max()));
                        a = new Range(a.min(), Math.min(a.max(), rule.threshold));
                    }
                    case "s" -> {
                        newPartRanges = newPartRanges.withS(new Range(Math.max(s.min(), rule.threshold + 1), s.max()));
        ...
        combinations += applyRules(new PartRanges(x, m, a, s), ruleSets.get(current.sendTo()), ruleSets, part1);
    ...
    public RuleSet parseRuleSet(String line) {
        ...
        for (String ruleStr : rulesStr) {
            if (ruleStr.contains(":")) {
                ...
                rules.add(new Rule(ruleStr.substring(0, 1), threshold, ruleStr.contains("<"), ruleStr.split(":")[1]));
            }
            else {
                sendTo = ruleStr;
            }

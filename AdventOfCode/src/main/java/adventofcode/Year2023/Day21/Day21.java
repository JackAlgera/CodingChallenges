package adventofcode.Year2023.Day21;
...
public class Day21 extends Day<Long> {
    ...
    public void bfs(String[][] grid, Pos current, int totalSteps, int maxSteps, Set<Pos> reachableSpots, Set<State> dp) {
        ...
        for (int k = 0; k < Utilities.NEIGHBORS_4_DIRECTIONS.length; k++) {
            ...
            int j = (next.j() < 0 ? grid[0].length + (next.j() % grid[0].length) : next.j()) % grid[0].length;

...
import adventofcode.utils.Vector3dLong;
...
public class Day24 extends Day<Long> {
    ...
    public Long part2(List<String> lines) throws IOException {
        ...
        RealMatrix mat = MatrixUtils.createRealMatrix(new double[][] {
        ...
        RealVector realH = MatrixUtils.createRealVector(new double[] {
        ...
        return Math.round(solution.getEntry(0)) + Math.round(solution.getEntry(1)) + Math.round(solution.getEntry(2));
    }
    ...
    public Vector2dFloat getIntersection(Hailstone h1, Hailstone h2) {
        ...
        long u = ((h2.pos().y() - h1.pos().y()) * h2.vel().x() - (h2.pos().x() - h1.pos().x()) * h2.vel().y()) / det;
        long v = ((h2.pos().y() - h1.pos().y()) * h1.vel().x() - (h2.pos().x() - h1.pos().x()) * h1.vel().y()) / det;
        if (u < 0 || v < 0) {
            return null;
        }
    ...
    public List<Hailstone> parseInput(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            hailstones.add(new Hailstone(
                new Vector3dLong(Long.parseLong(posStr[0].strip()), Long.parseLong(posStr[1].strip()), Long.parseLong(posStr[2].strip())),
                new Vector3dLong(Long.parseLong(velStr[0].strip()), Long.parseLong(velStr[1].strip()), Long.parseLong(velStr[2].strip()))));
        }
        return hailstones;
    }

...
public class Day16 extends Day<Long> {
    ...
    public Long part2(List<String> lines) throws IOException {
        ...
        tickets = tickets.stream()
            .filter(ticket -> ticket.isValid(fields))
            .toList();

        List<ArrayList<Integer>> fieldIndexesList = IntStream.range(0, fields.size())
        ...
        return Arrays.stream(finalIndexes).limit(6)
    ...
    public Ticket parseInput(List<String> lines, List<Field> fields, List<Ticket> tickets) {
        ...
        while (!lines.get(i).isEmpty()) {
            ...
            fields.add(new Field(line.split(":")[0], Arrays.stream(line
                .split(": ")[1]
                .split(" or "))
                .map(range -> new Range(
        ...
        i += 2;
        Ticket myTicket = new Ticket(Arrays.stream(lines.get(i).split(","))
            .map(Integer::parseInt)
            .toList());
        i += 3;
        while (i < lines.size()) {
            String line = lines.get(i);
            tickets.add(new Ticket(Arrays.stream(line.split(","))
                .map(Integer::parseInt)
                .toList()));
            i++;
        }

...
public class Day24 extends Day<Integer> {
    ...
    public Integer part2(List<String> lines) throws IOException {
        ...
        return getShortestPath(
            List.of(start, end, end, start, start, end),
            dimensions,
            lines
        );
    }
    ...
    private int getShortestPath(List<Vector> positions, Vector dimensions, List<String> lines) {
        ...
        for (int k = 0; k < positions.size() / 2; k++) {
            ...
            while (!playerPositions.contains(end)) {
                blizzardPositions = updateBlizzardPositions(blizzardPositions, blizzardDirections, dimensions);
                playerPositions = updatePlayerPositions(end, dimensions, playerPositions, blizzardPositions);
                time++;
            }
        }
    ...
    private static List<Vector> updateBlizzardPositions(List<Vector> blizzardPositions, List<Vector> blizzardDirections, Vector dimensions) {
        ...
        for (int k = 0; k < blizzardPositions.size(); k++) {
            ...
            updatedBlizzardPositions.add(new Vector(
